...
void loop() {
  ...
  client.stop();
  delay(1000);
  floyd_steinberg_dithering_serial(image_data, true_w, h);
  ...
  uint8_t *my_bitmap = new uint8_t[(int) (true_w / 8) * h] {};
  ...
  for (int i = 0, j = 0, bit_index = 7, column_index = 0; i < true_w * h; i++) {
    //uint8_t c = BitsSetTable256[image_data[i] & 0xff];
    if (fpga_data[i] == 0) {
      my_bitmap[j] |= (uint8_t) (1 << bit_index);
    }
  ...
  printer.printBitmap(true_w, h, my_bitmap, false);
