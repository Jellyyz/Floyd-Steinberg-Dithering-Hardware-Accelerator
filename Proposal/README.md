...
Then, we have the final time needed to print the image on the thermal printer, which will take O(xy*c) time. This is because printers print out rows, and for a fixed row, they print from one side of the paper to the other side of the paper. The following computes for c.
